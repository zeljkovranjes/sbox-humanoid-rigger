...
/// <summary>Both native file drop and the file dialog enter here. Format adapters return the same mesh contract.</summary>
public static class ModelImporter
{
    public static bool CanImport(string path) => string.Equals(Path.GetExtension(path),".fbx",StringComparison.OrdinalIgnoreCase);
    public static ImportedCharacter Import(string path)
    {
        if (!CanImport(path)) throw new FormatException("Select an FBX model.");
    ...
    public static ImportedCharacter Import(byte[] bytes, string name,string sourcePath="")
    {
        ...
        foreach(var geo in scene.ObjectsById.Values.Where(o=>o.NodeType=="Geometry" && o.SubClass=="Mesh"))
        {
            ...
            foreach(var model in instances.Length==0 ? new FbxObject?[]{null} : instances)
            {
                ...
                parts.Add(new(partName,vertices,triangles,Classify(partName)){CornerNormals=normals,CornerTexCoords=uvs,CornerColors=colors,TriangleMaterials=faceMaterials});
            }
        }
    ...
    static MeshKind Classify(string name)
    {
