...
public static class SurfaceRepair
{
    ...
    internal static ValidationReport TryWeights(ImportedCharacter character,GeneratedRig rig,ValidationReport initial,Influence[][][] weights)
        =>Improve(character,rig,initial,weights);

    static ValidationReport Improve(ImportedCharacter character,GeneratedRig rig,ValidationReport initial,Influence[][][]? proposed)
    {
        if(proposed is null&&!initial.Passed||initial.StressTests.All(t=>t.ReversedTriangles==0))return initial;
        ...
        var poses=specifications.Select((spec,i)=>
        {
            ...
            for(int p=0;p<faces.Length;p++)for(int t=0;t<faces[p].Length;t++)
            {
                ...
                if(Alignment(f,normal,rig.Weights[p],pose)<SurfaceOrientation.ReversalLimit)pose.Reversed.Add((p,t));
            }
            return pose;
        }).ToArray();
            ...
            for(int phase=0;proposed is null&&phase<8;phase++)
            {
                ...
                for(int round=0;round<(transfer?TransferRounds:4)&&trials<budget;round++)
                {
                    ...
                    for(int part=0;part<faces.Length&&trials<budget;part++)
                    {
                        ...
                        while(pending.Count>0&&trials<budget)
                        {
                            ...
                            if(transfer)
                            {
                                ...
                                foreach(var (v,weights) in Transfers(rig,part,core,neighbors[part]))
                                {
                                    if(trials>=budget||localTrials>=TransfersPerNeighborhood/2)break;trials++;localTrials++;
                                ...
                                else
                                {
                                    ...
                                    {
                                        if(trials>=budget||localTrials>=TransfersPerNeighborhood)break;trials++;localTrials++;
                            ...
                            foreach(float blend in new[]{.2f,.4f,.65f,.85f,1f})
                            {
                                ...
                                foreach(int v in region)
                                {
                                    ...
                                    candidate[v]=Skinning.Cleanup(values.Select((w,b)=>new Influence(b,w)),rig.Bones.Length,rig.Profile.MaximumInfluences);
        ...
        bool Local(Influence[] weights,Vector3 position)=>weights.All(w=>rig.Bones[w.Bone].Deform&&
            (w.Weight<=.05f||Vector3.Distance(position,Geometry.ClosestOnSegment(position,rig.Bones[w.Bone].Position,ends[w.Bone]))<=height*.25f));
    ...
    static IEnumerable<(int Vertex,Influence[] Weights)> Transfers(GeneratedRig rig,int part,HashSet<int> core,List<int>[] neighbors)
    {
    ...
    static IEnumerable<(int Vertex,Influence[] Weights)> VertexTransfers(GeneratedRig rig,int part,int v,List<int>[] neighbors)
    {
        ...
        foreach(var donor in rig.Weights[part][v].OrderByDescending(w=>w.Weight))foreach(int target in supported.Where(b=>b!=donor.Bone))
            foreach(float portion in new[]{.1f,.3f,.6f,1f})
            {
                ...
                yield return(v,Skinning.Cleanup(values.Select((w,b)=>new Influence(b,w)),rig.Bones.Length,rig.Profile.MaximumInfluences));
            }
    }
    static IEnumerable<(int A,int B,Influence[] First,Influence[] Second)> PairTransfers(GeneratedRig rig,int part,HashSet<int> core,List<int>[] neighbors)
    {
        ...
        foreach(float a in new[]{.1f,.3f,.6f,1f})foreach(float b in new[]{.1f,.3f,.6f,1f})foreach(var edge in edges)
    ...
    {
        gain=0;
        var updates=new List<Dictionary<int,Vector3>>();var reversals=new List<HashSet<int>>();double totalImprovement=0;
        foreach(var pose in poses)
        {
            ...
            foreach(int v in region)
            {
                ...
                if(!Geometry.Finite(value))return false;points.Add(v,value);
            }
            Vector3 Point(int v)=>points.TryGetValue(v,out var changed)?changed:pose.Points[part][v];
            double oldDeficit=0,newDeficit=0,oldVolume=0,newVolume=0;var reversed=new HashSet<int>();
            ...
            totalImprovement+=oldVolume-newVolume;updates.Add(points);reversals.Add(reversed);
        }
