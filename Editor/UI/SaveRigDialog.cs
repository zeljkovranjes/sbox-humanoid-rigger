...
public sealed class SaveRigDialog:Dialog
{
    ...
    readonly Checkbox fbx,vmdl;
    readonly LineEdit fileName,directory;
    readonly Label message;
    readonly Button save,browse,cancel;
    bool saving;
    internal ExportRequest Request=>new(fileName.Text,directory.Text,(fbx.Value?ExportFormats.Fbx:0)|(vmdl.Value?ExportFormats.Vmdl:0));
    ...
    public SaveRigDialog(Widget parent,Wizard session,Action<ExportResult> saved):base(parent)
    {
        ...
        fbx=formats.Add(new Checkbox("Fbx",this){Value=true});vmdl=formats.Add(new Checkbox("Vmdl",this){Value=true});formats.AddStretchCell();
        ...
        fbx.Toggled=ValidateRequest;vmdl.Toggled=ValidateRequest;fileName.TextChanged+=_=>ValidateRequest();directory.TextChanged+=_=>ValidateRequest();
        ValidateRequest();
    }
    void Browse()
    {
    ...
    void ValidateRequest()
    {
        ...
        try
        {
            ...
            message.Text=string.Join(" + ",plan.Files.Where(p=>!p.EndsWith(".dmx",StringComparison.OrdinalIgnoreCase)).Select(Path.GetFileName));
    ...
    void SetControlsEnabled(bool enabled){foreach(var widget in new Widget[]{fbx,vmdl,fileName,directory,browse,cancel,save})widget.Enabled=enabled;}
}
